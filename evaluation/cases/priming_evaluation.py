# evaluation/cases/priming_evaluation.py (Corrected)
...
class PrimingEvaluation(EvaluationCase):
    ...
    def _calculate_metrics_for_batch(self, batch: Dict[str, Any]) -> Dict[str, List[Dict[str, float]]]:
        ...
        # torch.chunk returns a tuple, so we will iterate by index instead of popping
        ...
        target_starts_map = {
            ...
            'logp_inconT_inconP': (batch['incon_target_start_in_incon_prime'], batch['incon_target_hotspot_start'],
                                   batch['incon_target_hotspot_end']),
            'logp_conT_base': (torch.full((original_batch_size,), 1, device=device), batch['con_target_hotspot_start'],
                               batch['con_target_hotspot_end']),
            'logp_inconT_base': (torch.full((original_batch_size,), 1, device=device),
                                 batch['incon_target_hotspot_start'], batch['incon_target_hotspot_end']),
        }

        log_probs_per_item = [defaultdict(float) for _ in range(original_batch_size)]

        for k, key in enumerate(target_starts_map.keys()):
            # --- THIS IS THE FIX: Access by index 'k' instead of using .pop() ---
            ...
            for i in range(original_batch_size):
                ...
                if hotspot_start_in_target != -1:
                    hotspot_start_in_seq = (target_start_idx - 1) + hotspot_start_in_target
                    hotspot_end_in_seq = (target_start_idx - 1) + hotspot_end_in_target
